...
test('should render - List < store', async (done) => {
  ...
  await load()
    .then(() => {
      ...
      const listNames = container.querySelectorAll('#list li a span.outer span>span:not(.unresponsive-sub)')
      ...
      const geawesserNames = container.querySelectorAll('#list li a span.outer span>span.unresponsive-sub')
