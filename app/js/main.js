...
if (d3.selectAll('#map').size() > 0) {
  ...
  d3.csv('./new_build.csv').then(function (data) {
    data.forEach(function (d, i) {
      gKeys[d.detail_id] = i
    })

    // https://flsshygn-lageso-berlin-prediction-merge-dev.s3.eu-central-1.amazonaws.com/app
    d3.csv('./data.csv?date=' + date.getYear() + '-' + date.getMonth() + '-' + date.getDate() + ':' + date.getHours()).then(function (apiData) {
      var mergeCols = [
        'prediction',
        'p_date',
        'wasserqualitaet',
...
function openDetails (id, zoom) {
  ...
  html += '<br /><br />' +
              ...
              '    <span class="stufen-icon stufen-' + data.real_state + '"></span>' + stufentext[data.real_state] + '<br /><span class="small">(Letzte Messung: ' + data.m_date + '<span id="lastPredict"></span>)</span>'
